...
class PocketToAccountPaymentController extends Controller
{
    //To get all the Wallet To Account payment data...
    public function getPocketToAccountPaymentData()
    {
        ///To get current user...
        $userId = CurrentUser::getUserId();

        //To get all the account payment data...
        $accountPaymentData = AccountPayment::orderBy('id','DESC')->where('payment_type', 'Wallet To Account')->where('user_id', $userId)
    ...
    public function getAccountDataWithBank(Request $request)
    {
        ...
        $accountData = Account::where('bank_id', $request->bank_id)->where('user_id', Auth::user()->id)
                        ->where('is_inactive', false)->with(['bankData','mobileWalletData'])->get();
