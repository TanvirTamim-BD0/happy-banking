...
@section('title') Mobile Wallet Accounts Details @endsection
@section('styles')
@endsection
@section('content')
    ...
    <div class="pay_card_wrap">
        <div class="pay_card mb-3 card">
            <div class="d-flex justify-content-between align-items-center mb-2 border-bottom pb-1">
                ...
                @if(isset($singleAccountData->mobileWalletData->image) && $singleAccountData->mobileWalletData->image != null)
                <img src="{{asset('backend/uploads/mobileWalletImage/thumbnail/'.$singleAccountData->mobileWalletData->image)}}"
                    alt="visa_card" />
                @else
                ...
                <div>
                    <h4 class="text-dark font-16 text-capitalize">{{$singleAccountData->mobileWalletData->mobile_wallet_name}}</h4>
                    <p class="text-dark font-13">Bank: {{$singleAccountData->mobileWalletData->parent_company}}</p>
                </div>
            </div>
    ...
    <div class="ds_body img-40 mb-2">
        ...
        <div class="d-flex flex-wrap flex-row card justify-content-between">
            <a class="border-end no-border-last w-50 gap-2 p-0 flex-column d-flex align-items-center"
                href="{{route('webuser.mobile-wallet-account-transaction-all-list',$singleAccountData->id)}}">
                <i class=" fa fa-table custom-dashboard-icon"></i>
                <p class="text-dark mb-0 pb-0">Transactions</p>
            </a>
            <a class="border-end no-border-last w-50 gap-2 p-0 flex-column d-flex align-items-center"
                href="{{route('webuser.payment-type-for-mobile-wallet-account-transfer',$singleAccountData->id)}}">
