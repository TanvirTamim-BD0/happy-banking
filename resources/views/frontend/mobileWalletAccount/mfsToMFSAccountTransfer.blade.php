...
@section('title') MFS To MFS Transfer @endsection
@section('styles')
@endsection
@section('content')
...
<div class="h-100 container">
    ...
    <div class="nav custom-transfer-acccount-tab gap-2 btn-no-color nav-tabs" id="nav-tab" role="tablist">
        ...
        <button class="primary_btn nav-link " id="nav-profile-tab" data-bs-toggle="tab" data-bs-target="#nav-profile" type="button"
                role="tab" aria-controls="nav-profile" aria-selected="false" onclick="getBeneficiaryMobileWalletData()">Beneficiary Account</button>
    </div>
    ...
    <div class="tab-content" id="nav-tabContent">
        <div class="tab-pane fade show active" id="nav-home" role="tabpanel" aria-labelledby="nav-home-tab">
            <form class="card mb-3" action="{{route('webuser.mfs-to-mfs-account-transfer',$accountData->id)}}" method="post">
                @csrf
            ...
            <div class="form-group custom-form-group">
                ...
                <div class="single_input "><i class="fa-solid fa-bangladeshi-taka-sign"></i><input name="pay_amount" type="number"
                    id="pay_amount" placeholder="Enter Amount" step="0.01" required></div>
            </div>
        ...
        <div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
            <form class="card mb-3" action="{{route('webuser.mfs-to-mfs-account-transfer',$accountData->id)}}" method="post">
                ...
                <div class="single_input custom-select2-dropdown">
                    <i class="fa-solid fa-piggy-bank"></i>
                    <select autocomplete="off" name="mobile_wallet_id" id="beneficiary_mobile_wallet_id" required class="form-control">
                        <option value="" selected disabled>Select Mobile Wallet</option>
                ...
                <label class="custom-form-label" for="email">Amount <span class="custom-danger">(requierd)</span></label>
                <div class="single_input "><i class="fa-solid fa-bangladeshi-taka-sign"></i><input name="pay_amount" type="number"
                    id="pay_amount" placeholder="Enter Amount" required></div>
                </div>
...
<script>
    ...
    function getBeneficiaryMobileWalletData()
    {   
        var url = "{{ route('webuser.get-beneficiary-mobille-wallet-account-data') }}";
            $.ajaxSetup({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                }
