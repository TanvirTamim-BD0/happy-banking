...
<div class="h-100 container">
    ...
    <div class="nav custom-transfer-acccount-tab gap-2 btn-no-color nav-tabs" id="nav-tab" role="tablist">
        <button class="primary_btn nav-link active" id="nav-home-tab" data-bs-toggle="tab" data-bs-target="#nav-home" type="button"
                role="tab" aria-controls="nav-home" aria-selected="true">Own Account</button>
        <button class="primary_btn nav-link " id="nav-profile-tab" data-bs-toggle="tab" data-bs-target="#nav-profile" type="button"
                role="tab" aria-controls="nav-profile" aria-selected="false" onclick="getBeneficiaryBankData()">Beneficiary Account</button>
    </div>
    ...
    <div class="tab-content" id="nav-tabContent">
        <div class="tab-pane fade show active" id="nav-home" role="tabpanel" aria-labelledby="nav-home-tab">
            <form class="card mb-3" action="{{route('webuser.mfs-to-bank-account-transfer',$accountData->id)}}"
                method="post">
                @csrf
        ...
        <div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
            <form class="card mb-3" action="{{route('webuser.mfs-to-bank-account-transfer',$accountData->id)}}" method="post">
                @csrf
                ...
                <div class="single_input custom-select2-dropdown">
                    <i class="fa-solid fa-piggy-bank"></i>
                    <select autocomplete="off" name="beneficiary_bank_id" id="beneficiary_bank_id" required class="form-control">
                        <option value="" selected disabled>Select Bank</option>
                ...
                <div class="single_input d-flex align-items-center gap-2 justify-content-around border">
                    <label class="d-flex py-2 align-items-center gap-2">
                        <input type="radio" name="transfer_channel" checked="">BEFTN</label>
                    <label class="d-flex py-2 border-end border-start px-4 align-items-center gap-2">
                        <input type="radio" name="transfer_channel">NPSB</label>
                    <label class="d-flex py-2 align-items-center gap-2">
                        <input type="radio" name="transfer_channel">RTGS</label>
                </div>
                </div> --}}
                ...
                <div class="form-group custom-form-group">
                    ...
                    <div class="single_input ">
                        <i class="fa-solid fa-bangladeshi-taka-sign"></i>
                        <input name="pay_fee" type="number" id="pay_fee_for_beneficiary" placeholder="Processing Fee(Max 5%)"
                            onblur="checkProcessingFeeBene()" required>
                    </div>
                </div>
...
<script>
    ...
    //To check processing fee...
    function checkProcessingFeeBene() {
        var payFee = $('#pay_fee_for_beneficiary').val();
    ...
    $("#bank_id").change(function () {
        var bankId = $(this).val();
        var url = "{{ route('webuser.get-account-data-bank-wise') }}";
        if (bankId != '') {
            $.ajaxSetup({
                headers: {
