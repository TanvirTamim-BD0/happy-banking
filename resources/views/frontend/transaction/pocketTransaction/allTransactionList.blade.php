...
@section('title') All Transaction List Of Pocket Wallet @endsection
@section('styles')
@endsection
@section('content')
...
<div class="h-100 container ">
    <div class="custom-transfer-acccount-tab gap-2 btn-no-color">
        <a href="{{route('webuser.pocket-wallet-account-transaction-all-list')}}" class="text-white primary_btn nav-link active">
            All
        </a>
        
        <a href="{{route('webuser.pocket-wallet-account-transaction-credit-list')}}" class="text-white primary_btn nav-link">
            Credit
        </a>
       
        <a href="{{route('webuser.pocket-wallet-account-transaction-debit-list')}}" class="text-white primary_btn nav-link">
            Debit
        </a>
    </div>
                                ...
                                <div class="d-flex flex-column custom-credit-debit-date-time">
                                    <p class="text-secondary fw-light">
                                        @if($singleItemForAll['type'] == 'Income/Expense')
                                            ...
                                            @endphp
                                            @if ($getSingleAccData != null && $getSingleAccData->income_expense_type == 'Banking Income')
                                        ...
                                        @else
                                            ...
                                            @endphp
                                            @if ($getSingleAccPayData != null && $getSingleAccPayData->to_pocket_account_id != null)
                                                <b>Transfer:</b> {{$singleItemForAll['credit_debit_amount']}} BDT
                                            @else
                                                <b>Transfer:</b> {{$singleItemForAll['credit_debit_amount']}} BDT
                                            @endif
                                        ...
                                        @if($singleItemForAll['type'] == 'Income/Expense')
                                            @php
                                                $getSingleAccData = App\Models\IncomeExpense::getSingleIncomeExpense($singleItemForAll['id']);
                                            @endphp
                                            @if ($getSingleAccData != null && $getSingleAccData->income_expense_type == 'Pocket Wallet Income')
