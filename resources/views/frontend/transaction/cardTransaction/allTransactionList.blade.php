...
@section('title') All Transaction List Of Credit Card Wallet @endsection
@section('styles')
@endsection
@section('content')
...
<div class="h-100 container ">
    <div class="custom-transfer-acccount-tab gap-2 btn-no-color">
        <a href="{{route('webuser.card-wallet-account-transaction-all-list', $accountId)}}" class="text-white primary_btn nav-link active">
            All
        </a>
        
        <a href="{{route('webuser.card-wallet-account-transaction-credit-list', $accountId)}}" class="text-white primary_btn nav-link">
            Credit
        </a>
                                ...
                                <div class="d-flex flex-column custom-credit-debit-date-time">
                                    <p class="text-secondary fw-light">
                                        ...
                                        @else
                                            ...
                                            @endphp
                                            @if ($getSingleAccPayData->to_credit_card_id != null && $getSingleAccPayData->to_credit_card_id == $accountId)
                                                <b>Transfer:</b> {{$singleItemForAll['credit_debit_amount']}} BDT
                                            @else
                                                <b>Transfer:</b> {{$singleItemForAll['credit_debit_amount']}} BDT
                                            @endif
                            ...
                            <div class="bg-secondary p-2">
                                @if($singleItemForAll['type'] == 'Income/Expense')
                                    @php
                                        $getSingleAccData = App\Models\IncomeExpense::getSingleIncomeExpense($singleItemForAll['id']);
                                    @endphp
                                    @if ($getSingleAccData != null && $getSingleAccData->income_expense_type == 'Credit Card Income')
