...
@section('title') Banking Income Create @endsection
@section('styles')
@endsection
@section('content')
...
<div class="h-100 container">
    ...
    <div class="tab-content" id="nav-tabContent">
        <div class="tab-pane fade show active" id="nav-home" role="tabpanel" aria-labelledby="nav-home-tab">
            <form class="card mb-3" action="{{route('webuser.wallet-income-save',['income_type' => Crypt::encrypt("Banking Income")])}}"
                method="post">
                @csrf
    
                <input type="hidden" id="incomeType" name="income_expense_type" value="{{$incomeType}}">
                ...
                <button class="primary_btn mb-1">Add Income</button>
            </form>
        </div>
    </div>
...
<script>
    ...
    $("#bank_id").change(function () {
        var bankId = $(this).val();
        var url = "{{ route('webuser.get-account-bank-wise') }}";
        if (bankId != '') {
            $.ajaxSetup({
                headers: {
